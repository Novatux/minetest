...
void DungeonGen::generate(u32 bseed, v3POS nmin, v3POS nmax) {
...
{
	v3POS areasize = vm->m_area.getExtent();
	v3POS roomsize;
	v3POS roomplace;
	...
	*/
	...
	for (u32 i = 0; i < 100 && !fits; i++)
	{
		...
		roomsize = is_large_room ?
			v3POS(random.range(8, 16),random.range(8, 16),random.range(8, 16)) :
			v3POS(random.range(4,  8),random.range(4,  6),random.range(4, 8));
		roomsize += dp.roomsize;

		// start_padding is used to disallow starting the generation of
		// a dungeon in a neighboring generation chunk
		roomplace = vm->m_area.MinEdge + start_padding + v3POS(
	...
	*/
	...
	for (u32 i = 0; i < room_count; i++)
	{
		...
		if (mg->gennotify & (1 << dp.notifytype)) {
			std::vector <v3POS> *nvec = mg->gen_notifications[dp.notifytype];
		...
		roomsize = v3POS(random.range(4,8),random.range(4,6),random.range(4,8));
		roomsize += dp.roomsize;
...
void DungeonGen::makeCorridor(v3POS doorplace,
		v3POS doordir, v3POS &result_place, v3POS &result_dir)
{
	makeHole(doorplace);
	v3POS p0 = doorplace;
	v3POS dir = doordir;
	...
	for (u32 i = 0; i < length; i++) {
		...
		if (vm->m_area.contains(p) == true &&
			vm->m_area.contains(p + v3POS(0, 1, 0)) == true) {
			if (make_stairs) {
				makeFill(p + v3POS(-1, -1, -1), dp.holesize + v3POS(2, 3, 2),
				...
				makeFill(p + v3POS(-1, -1, -1), dp.holesize + v3POS(2, 2, 2),
						VMANIP_FLAG_DUNGEON_UNTOUCHABLE, MapNode(dp.c_cobble), 0);
				makeHole(p);
			}
...
bool DungeonGen::findPlaceForDoor(v3POS &result_place, v3POS &result_dir)
{
	for (u32 i = 0; i < 100; i++)
	{
		...
		*/
		...
		if (vm->getNodeNoExNoEmerge(p+v3POS(0,0,0)).getContent() == dp.c_cobble
		 && vm->getNodeNoExNoEmerge(p+v3POS(0,1,0)).getContent() == CONTENT_AIR
		 && vm->getNodeNoExNoEmerge(p+v3POS(0,2,0)).getContent() == CONTENT_AIR)
			p += v3POS(0,1,0);
		// Jump one down if the actual space is there
		if (vm->getNodeNoExNoEmerge(p+v3POS(0,1,0)).getContent() == dp.c_cobble
		 && vm->getNodeNoExNoEmerge(p+v3POS(0,0,0)).getContent() == CONTENT_AIR
		 && vm->getNodeNoExNoEmerge(p+v3POS(0,-1,0)).getContent() == CONTENT_AIR)
			p += v3POS(0,-1,0);
...
#if 0
		if (doordir == v3POS(1, 0, 0)) // X+
			roomplace = doorplace + v3POS(0, -1, -roomsize.Z / 2);
		if (doordir == v3POS(-1, 0, 0)) // X-
			roomplace = doorplace + v3POS(-roomsize.X+1,-1,-roomsize.Z / 2);
		if (doordir == v3POS(0, 0, 1)) // Z+
			roomplace = doorplace + v3POS(-roomsize.X / 2, -1, 0);
		if (doordir == v3POS(0, 0, -1)) // Z-
			roomplace = doorplace + v3POS(-roomsize.X / 2, -1, -roomsize.Z + 1);
#endif
...
v3POS rand_ortho_dir(PseudoRandom &random, bool diagonal_dirs)
{
	...
	if (diagonal_dirs && (random.next() % 4 == 0)) {
		...
		do {
			trycount++;
			dir = v3POS(random.next() % 3 - 1, 0, random.next() % 3 - 1);
